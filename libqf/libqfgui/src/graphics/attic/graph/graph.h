...
#ifndef QF_GUI_GRAPHICS_GRAPH_H
#define QF_GUI_GRAPHICS_GRAPH_H
...
class QFGUI_DECL_EXPORT Graph
{
	...
	protected:
		...
		qreal x2device(qreal x) {return qf::gui::graphics::x2device(x, painter()->device());}
		qreal y2device(qreal y) {return qf::gui::graphics::y2device(y, painter()->device());}
		qreal device2x(qreal x) {return qf::gui::graphics::device2x(x, painter()->device());}
		qreal device2y(qreal y) {return qf::gui::graphics::device2y(y, painter()->device());}
		Point mm2device(const Point &p) {return qf::gui::graphics::mm2device(p, painter()->device());}
		Rect mm2device(const Rect &r) {return qf::gui::graphics::mm2device(r, painter()->device());}
		Point device2mm(const Point &p) {return qf::gui::graphics::device2mm(p, painter()->device());}
		Rect device2mm(const Rect &r) {return qf::gui::graphics::device2mm(r, painter()->device());}
	...
	public:
		...
		void setStyleCache(const qf::gui::graphics::StyleCache &cache) {d->styleCache = cache;}
...
class QFGUI_DECL_EXPORT HistogramGraph : public Graph
{
private:
	typedef Graph Super;
	protected:
...
class QFGUI_DECL_EXPORT PieGraph : public Graph
{
private:
	typedef Graph Super;
protected:
