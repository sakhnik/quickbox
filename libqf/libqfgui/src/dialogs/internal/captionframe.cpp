...
using namespace qf::gui::dialogs::internal;
...
QIcon CaptionFrame::createIcon()
{
	...
	if(!icon_path.isEmpty()) {
		if(!icon_path.startsWith(QLatin1String(":/")) && !icon_path.startsWith('/')) {
			qf::gui::framework::Plugin *plugin = qf::gui::framework::MainWindow::frameWork()->pluginForObject(this);
			if(!plugin) {
		...
		if(ico.isNull())
			ico = QIcon(":/qf/gui/images/under-construction.png");
	}
	return ico;
}
...
{
	qfLogFuncFrame() << "mode:" << qf::gui::model::DataDocument::recordEditModeToString(qf::gui::model::DataDocument::RecordEditMode(mode));
...
void CaptionFrame::update()
{
	...
	if(m_recordEditMode >= 0) {
		auto mode = qf::gui::model::DataDocument::RecordEditMode(m_recordEditMode);
	...
	if(is_visible) {
		if(isAlert()) {
			QPixmap pm = qf::gui::Style::instance()->pixmap(":/qf/gui/images/alert", 32);
